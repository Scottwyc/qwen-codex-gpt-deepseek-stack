...
class SseTranslator:
    def __init__(self, model: Optional[str] = None, response_prefix: str = "resp_ds"):
        self.model = model or MODEL
        # Use a DS-specific response prefix so a future GPT request can tell that
        # `previous_response_id` points to a synthetic local/DeepSeek response,
        # not to a ChatGPT backend response id.
        self.response_id = _rand_id(response_prefix)
    ...
    def warn(self, msg: str) -> str:
        """向 codex 发送可见的警告提示（不终止流，允许后续继续输出）"""
        out = self._ensure_started()
        if not self.text_started:
            # 如果还没有开始输出文本，先初始化文本 item
            self.text_started = True
            oi = self.output_item_count
            self.output_item_count += 1
            self.output_items.append({
        ...
        out += self._emit(
            "response.output_text.delta",
            {
                ...
                "response_id": self.response_id,
                "item_id": self.message_item_id,
                "output_index": self.output_item_count - 1,
                "content_index": 0,
                "delta": f"\n⚠️ {msg}\n",
            },
        )
        log.warn("SSE warn: " + msg)
        return out

    def error(self, msg: str) -> str:
        # 必须先发 response.created + response.in_progress，
        # 否则 Codex 会一直等待 response 生命周期开始
        out = self._ensure_started()
        out += self._emit(
            "error", {"type": "error", "code": "proxy_error", "message": msg}
        )
        # 必须发送 response.completed，否则 Codex 认为流被截断，触发 Reconnecting
        out += self._emit(
            "response.completed",
            {
                "type": "response.completed",
